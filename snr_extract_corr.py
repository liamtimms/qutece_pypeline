...
for sub_num in subject_list:
    data_dir = os.path.join(base_path, 'derivatives', 'datasink',
                            'preprocessing', 'sub-' + sub_num, 'ses-Post',
                            'qutece')
    json_dir = os.path.join(base_path, 'sub-' + sub_num, 'ses-Post', 'qutece')
    ...
    for filename in os.listdir(data_dir):
        if filename.endswith(".nii"):
            ...
            fname = fname.replace('_corrected', '')
            ...
            cnr = snr - snr_tissue
            scan_vals = [fname, signal, snr, cnr, voxelsize, TE, TR, FA]
            sub_vals.append(scan_vals)
            print('for ' + snr_fname)
            print('SNR = ' + str(snr))
            print('CNR = ' + str(cnr) + '\n')
            del signal, noise, tissue
            n = n + 1

        scan_vals_csv_filename = os.path.join(
            snr_mask_dir, 'snr_sub-' + sub_num + '_corrected.csv')
