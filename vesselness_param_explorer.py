...
def parameter_test(scan_img, mask_img, suppressBlobs_list, suppressPlates_list,
    ...
    for params in params_space:
        ...
        t = 'equal'
        r = 1
        scan_img, __ = roi_cut(scan_img, mask_img, t, r)
        ...
        no_vess = (vessel_img < 0.1).astype(int) * 1
        # So we have
        ...
        summary_df = vess_roi_extract(scan_img, vessel_roi_img)
        df = summary_df.T
...
def main():
    ...
    infile = 'rsub-11_ses-Postcon_hr_run-01_UTE_desc-preproc.nii'
    ...
    # gamma_list =[0.01 * mean, 0.25 * mean, 0.5 * mean, 0.75 * mean, 1 * mean]
    gamma_list = [0.01 * mean, 0.25 * mean]
    ...
    full_df = parameter_test(scan_img, mask_img, suppressBlobs_list,
                             suppressPlates_list, gamma_list, sigma_max_list,
                             sigma_step_list)

    save_dir = os.path.join(manualwork_dir, 'vesselness_optimization')
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    save_name = 'vesselness_testing.csv'
